...
import { fetchShalomAvailableDeliveryDates } from '../api/rainauDelivery'
...
export function useShalomAvailableDeliveryDates(enabled, { pollMs = DEFAULT_POLL_MS, fastPoll = false } = {}) {
  ...
  const refresh = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const response = await fetchShalomAvailableDeliveryDates({ bustCache: true })
      if (requestIdRef.current !== requestId) return

      if (!response.success) {
        throw new Error(response.message || 'No se pudieron cargar las fechas de envío Shalom')
      }
      ...
      if (requestIdRef.current !== requestId) return
      if (!silent) {
        setError(fetchError.message || 'No se pudieron cargar las fechas de envío Shalom')
      }
    } finally {
